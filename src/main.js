...
// Initialize the game
function initGame() {
    // Create the state machine using XState v5 API
    const stateMachine = createMachine({
        ...
        states: {
            ...
            hurt: {
                ...
                after: {
                    1000: [
                        { target: 'move', guard: () => Math.abs(velocity.x) > 0.1 || Math.abs(velocity.y) > 0.1 },
                        { target: 'idle' }
                    ]
                }
            },
    ...
    // Create the state machine actor using XState v5 API
    stateActor = createActor(stateMachine);
    
    stateActor.subscribe((snapshot) => {
        // Update UI to show current state
        document.getElementById('currentState').textContent = snapshot.value;
        
        // Update animation when state changes
        console.log(`State transition to: ${snapshot.value}`);
        updateAnimation(snapshot.value);
    });
...
function handleKickAction() {
    ...
    if (stateActor.getSnapshot().value !== 'kick') {
        stateActor.send({ type: 'KICK' });
        ...
        kickTimeout = setTimeout(() => {
            if (isKicking) {
                ...
                stateActor.send({ type: isMoving ? 'MOVE' : 'IDLE' });
            }
        }, 300);
    }
}
...
function checkCollisions() {
    ...
    obstacles.forEach(obstacle => {
        ...
        if (checkHitboxCollision(playerHitbox, obstacleHitbox)) {
            ...
            if (stateActor.getSnapshot().value !== 'hurt') {
                ...
                setTimeout(() => {
                    ...
                    if (stateActor.getSnapshot().value === 'hurt') {
                        if (Math.abs(velocity.x) > 0.1 || Math.abs(velocity.y) > 0.1) {
                            stateActor.send({ type: 'MOVE' });
                        } else {
                            stateActor.send({ type: 'IDLE' });
                        }
                    }
                }, 1000);
            }
...
function updatePhysics() {
    ...
    // Get current state
    const currentState = stateActor.getSnapshot().value;
        ...
        if (frameIndex > currentAnim.end) {
            ...
            if (currentAnim.loop === false) {
                ...
                setTimeout(() => {
                    ...
                    stateActor.send({ type: nextState });
...
function updateSpriteFrame() {
    ...
    // Apply the sprite image and position - use Vite's asset handling
    sprite.style.backgroundImage = `url('/DinoSprites - ${currentCharacter}.png')`;
...
function changeCharacter(character) {
    ...
    if (stateActor) {
        updateAnimation(stateActor.getSnapshot().value);
    } else {
...
function preloadSprites() {
    ...
    characters.forEach(character => {
        const img = new Image();
        img.src = `/DinoSprites - ${character}.png`; // Vite will serve from public/
...
window.testSneakAnimation = testSneakAnimation;
